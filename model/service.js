...
export const createService = async ({
  ...
  longitude = null,
    ...
    console.log('createService', title, serviceDescription, authorUser, providerUser, serviceType, latitude, longitude);
  const { rows } = await pool.query(
    `INSERT INTO Service(
            title, 
            serviceDescription, 
...
export const updateService = async (
  serviceID,
  { title, serviceDescription, providerUser, latitude, longitude }
) => {
...
export const deleteService = async (serviceID, pool) => {
  return await pool.query("DELETE FROM Service WHERE serviceID = $1", [
    serviceID,
  ]);
};
