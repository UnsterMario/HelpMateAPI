...
export const getAboutUs = async (clientSQL) => {
    const {rows} = await clientSQL.query('SELECT content FROM AboutUs');
    return rows[0];
};

export const updateAboutUs = async (clientSQL, {content}) => {
    const {rows} = await clientSQL.query('UPDATE AboutUs SET content = $1 RETURNING last_updated', [content]);
    return rows[0];
};
